...
namespace AdvancedBot.Controls
{
    public class UserListBox : ScrollableControl
    {
        ...
        public HashSet<MinecraftClient> SelectedItems = new HashSet<MinecraftClient>();
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            for (int i = Math.Max(0, -(AutoScrollPosition.Y / ITEM_HEIGHT) - 1), ul = Math.Min(Items.Count, i + (h / ITEM_HEIGHT) + 3) + io; i < ul; i++) {
                ...
                if(io == 1 && i == 0) {
                    ...
                    isSelected = Selected == cli || SelectedItems.Contains(cli);
                }
                if(isSelected) {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left) {
                Focus();
                SetSelected(e.Y, EventType.MouseDown);
            }
        }
        protected override void OnMouseMove(MouseEventArgs e)
        {
            base.OnMouseMove(e);
            if(e.Button == MouseButtons.Left) {
                SetSelected(e.Y, EventType.MouseMove);
            }
        }
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (n != 0) {
                SetSelected((lastSelectedIndex + n) * ITEM_HEIGHT + AutoScrollPosition.Y, EventType.KeyDown);
            } else if (e.KeyCode == Keys.A && (e.Modifiers & Keys.Control) != 0) {
                foreach(var cli in Items) {
                    SelectedItems.Add(cli);
                }
                Selected = null;
                Invalidate();
        ...
        private void SetSelected(int y, EventType type)
        {
            int io = Items.Count > 1 ? 1 : 0;
            int i = (y - AutoScrollPosition.Y) / ITEM_HEIGHT;

            bool allowMulti = type == EventType.MouseDown || type == EventType.MouseMove;
            bool ctrl = IsKeyDown(Keys.ControlKey);
            if(allowMulti && !ctrl) {
                SelectedItems.Clear();
            } else if(allowMulti && ctrl && Selected is MinecraftClient selCli && SelectedItems.Count == 0) {
                SelectedItems.Add(selCli);
            }
            ...
            if (io == 1 && i == 0) {
                ...
                var item = Items[i - io];
                if (allowMulti && ctrl) {
                    if(!SelectedItems.Add(item) && type == EventType.MouseDown) {
        ...
        private static bool IsKeyDown(Keys key) => (GetKeyState(key) & 0x100) != 0;
